...
import dataFacade.PersonFacade;
...
public class PersonResource {
    ...
    public Response getPersonByID(@PathParam("id") int id) throws NotFoundException {
        pfacade.addEntityManagerFactory(Persistence.createEntityManagerFactory("pu2"));
    ...
    public Response updatePerson(String content, @PathParam("id") int id) {
        ...
        if (newPerson.getFirsName() != null) {
            savedPers.setFirstname(newPerson.getFirsName());
        }
        if (newPerson.getLastName() != null) {
            savedPers.setLastname(newPerson.getLastName());
        }
        pfacade.updatePerson(newPerson);
